...
def TakeImages():
    ...
    if ((name.isalpha()) or (' ' in name)):
        cam = csv.VideoCapture(0)
        harcascadePath = "haarcascade_frontalface_default.xml"
        detector = csv.CascadeClassifier(harcascadePath)
        sampleNum = 0
        while (True):
            ret, img = cam.read()
            gray = csv.cvtColor(img, csv.COLOR_BGR2GRAY)
            faces = detector.detectMultiScale(gray, 1.3, 5)
            for (x, y, w, h) in faces:
                csv.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
                # incrementing sample number
                sampleNum = sampleNum + 1
                # saving the captured face in the dataset folder TrainingImage
                csv.imwrite("TrainingImage\ " + name + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
                csv.imshow('Taking Images', img)
            # wait for 100 miliseconds
            if csv.waitKey(100) & 0xFF == ord('q'):
                break
...
def TrainImages():
    check_haarcascadefile()
    assure_path_exists("TrainingImageLabel/")
    recognizer = csv.face_LBPHFaceRecognizer.create()
    harcascadePath = "haarcascade_frontalface_default.xml"
    detector = csv.CascadeClassifier(harcascadePath)
...
def TrackImages():
    ...
    i = 0
    j = 0
    recognizer = csv.face.LBPHFaceRecognizer_create()  # cv2.createLBPHFaceRecognizer()
    ...
    if exists3:
        ...
        return
    harcascadePath = "haarcascade_frontalface_default.xml"
    faceCascade = csv.CascadeClassifier(harcascadePath);

    cam = csv.VideoCapture(0)
    font = csv.FONT_HERSHEY_SIMPLEX
    ...
    while True:
        ret, im = cam.read()
        gray = csv.cvtColor(im, csv.COLOR_BGR2GRAY)
        faces = faceCascade.detectMultiScale(gray, 1.2, 5)
        for (x, y, w, h) in faces:
            csv.rectangle(im, (x, y), (x + w, y + h), (225, 0, 0), 2)
            ...
            if (conf < 50):
                ...
                Id = 'Unknown'
                bb = str(Id)
            csv.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 255), 2)
        csv.imshow('Taking Attendance', im)
        if (csv.waitKey(1) == ord('q')):
            break
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%Y')
...
exists = os.path.isfile("StudentDetails\StudentDetails.csv") 
if exists:
    with open("StudentDetails\StudentDetails.csv") as csvFile1:
        reader1 = csv.reader(csvFile1)
        for l in reader1:
            res = res + 1
    res = (res // 2) - 1
